...
public class GetAllItems extends JFrame {
    ...
    public GetAllItems(SortedMap<Integer, SupplierItem> items) {
        this.service = service;

        setTitle("Get Suppliers Items Information");
    ...
    private static DefaultTableModel getTableModel(SortedMap<Integer, SupplierItem> items) {
        String[] columnNames = {"Supplier Name", "Quantity", "Item id", "Catalog",
                "Expiration", "Price",
                "Discount", "Manufacture"};
      ...
      for(Integer s:items.keySet()){
          try {
              SupplierItem supplier = items.get(s);
              data[i][0] = supplier.getSupplier();
              data[i][1] = supplier.getQuantity();
              data[i][2] = supplier.getItemId();
              data[i][3] = supplier.getSupplier_catalogID();
              data[i][4] = supplier.getExpiration();
              data[i][5] = supplier.getItem_list_price();
              data[i][6] = supplier.getDiscountByQuantity();
              data[i][7] = supplier.getItemManufacture();
          }
          catch (Exception e){
    ...
    private void initComponents(SortedMap<Integer, SupplierItem> items) {
