...
public class GetAllIDiscount extends JFrame {
    ...
    public GetAllIDiscount(List<Discount> items) {
        this.service = service;

        setTitle("Get Discount Information");
    ...
    private static DefaultTableModel getTableModel(List<Discount> items) {
        String[] columnNames = { "Sale number","Sale name", "Discount", "End date",
                "Start date","Include items"};

        Object[][] data = new Object[items.size()][columnNames.length];
        System.out.println(items);
        int i=0;

      for(Discount item:items){
          try {
              data[i][0] = item.getSaleNumber();
              data[i][1] = item.getSaleName();
              data[i][2] = item.getDiscount();
              data[i][3] = item.getEndDate();
              data[i][4] = item.getStartDate();
              data[i][5] = item.getIncludeInDiscount();
          }
          catch (Exception e){
    ...
    private void initComponents(List<Discount> items) {
